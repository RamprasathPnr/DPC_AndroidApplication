...
public class BlueToothPrint {
    ...
    private BluetoothConnector blue;
...
//                        Toast.makeText(context, "Bluetooth turned  off", Toast.LENGTH_SHORT).show();
    ...
    public BlueToothPrint(Activity context) {
        exception_count=0;
...
//    }
    ...
    public void opendialog(String printdata) {
        ...
        String device_address = MySharedPreference.readString(context, MySharedPreference.DEVICE_ADDRESS, "");
...
//            dialog(devicelist, R.layout.bluetooth_device_alert, context.getResources().getString(R.string.title_bluetooth_devices));
    ...
    private class ConnectRunnable extends Thread {
        public void run() {
            blue = new BluetoothConnector(context, mmDevice, false, mBluetoothAdapter, null);
            try {
                BluetoothConnector.BluetoothSocketWrapper socketwrapper = blue.connect();
...
//                    mmDevice = mBluetoothAdapter.getRemoteDevice(mmDevice.getAddress());
                    Log.e("selected device name", mmDevice.getName());
