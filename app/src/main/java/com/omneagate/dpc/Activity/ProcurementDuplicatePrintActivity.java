...
public class ProcurementDuplicatePrintActivity extends BaseActivity implements View.OnClickListener, ConnectivityReceiver.ConnectivityReceiverListener {

    private String TAG = "ProcurementDuplicatePrintActivity";
    ...
    private Button btn_print, btn_lose;
    ...
    private void setUpView() {
        ...
        dpcProcurementDto = (DPCProcurementDto) getIntent().getSerializableExtra("procurement_dto");
        Long farmer_id = dpcProcurementDto.getFarmerRegistrationDto().getId();
        Log.e("FARMER ID", "" + farmer_id);
        farmer_registration_dto = DBHelper.getInstance(ProcurementDuplicatePrintActivity.this).getFarmerDetailsProcurementDuplicate(farmer_id);
        TextView tv_procurement_receipt_number = (TextView) findViewById(R.id.tv_procurement_receipt_number);
        tv_procurement_receipt_number.setText(dpcProcurementDto.getProcurementReceiptNo());
    ...
    private void setdetails() {
        ...
        try {
            if (dpcProcurementDto != null) {
                ...
                dpc_dto = DBHelper.getInstance(ProcurementDuplicatePrintActivity.this).getDpcProfileById(id);
                if (dpc_dto != null) {
            ...
            if (dpcProcurementDto != null) {
                ...
                paddy_name = DBHelper.getInstance(ProcurementDuplicatePrintActivity.this).getpaddy(paddy_id);
            }

            if (dpcProcurementDto != null) {
                long grad_id = dpcProcurementDto.getPaddyGradeDto().getId();
                grade_dto = DBHelper.getInstance(ProcurementDuplicatePrintActivity.this).getGradeById(grad_id);
            }

            if (dpcProcurementDto != null) {
                rate_dto = DBHelper.getInstance(ProcurementDuplicatePrintActivity.this).getRate(dpcProcurementDto.getPaddyGradeDto().getId());
            }

            if (dpcProcurementDto != null) {
                long specification_id = dpcProcurementDto.getDpcSpecificationDto().getId();
                specification_type = DBHelper.getInstance(ProcurementDuplicatePrintActivity.this).getSpecificationById(specification_id);
            }
            ...
            Date myDate = null;
            try {
                myDate = dateFormat.parse(dpcProcurementDto.getTxnDateTime());
